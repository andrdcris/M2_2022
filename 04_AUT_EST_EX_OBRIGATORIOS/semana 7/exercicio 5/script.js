const calc = () => {
    ...
    for (let i = 0; i < numero; i++) {
        saida += `<div id="div${i}">
        <input type="number" placeholder="prova" id="prova${i}">
        <input placeholder="trabalho" type="number" id="trabalho${i}">
        </div>`;
    }
    $("#saida").append(
...
const calcular = () => {
    ...
    for (let i = 0; i < numero; i++) {
        ...
        todasasnotasprovas.sort(function (a, b) {
          ...
          $(`#saida2`).append(
            `<span>A média geral dos alunos é ${valor}, já a média das provas foi de ${valorprova}. 
            <br> A média dos trabalhos foi de: ${valortrabalho}, a maior nota da prova foi ${maiornotaprova}
             e a menor foi ${menornotaprova} <br> Já nos trabalhos, a maior nota foi de ${maiornotatrabalho}
              e a menor foi de ${menornotatrabalho}</span>`
          );
    };